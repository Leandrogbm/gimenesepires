  ...
  return (
    ...
    >
      <div className="mx-auto max-w-6xl px-6 md:px-10 flex items-center justify-between h-20">
        <a href="#topo" className="font-display text-xl tracking-tight text-ink">
          Gimenes <span className="text-wine italic">&amp;</span> Pires
        </a>

        <nav className="hidden md:flex items-center gap-9 font-body text-[15px] text-ink/80">
          {LINKS.map((l) => (
          ...
          <a
            href="#contato"
            className="border border-ink text-ink px-5 py-2 hover:bg-ink hover:text-paper transition-colors"
          >
            Fale conosco
          </a>
        </nav>
      ...
      {open && (
        <nav className="md:hidden bg-paper border-t border-ink/10 px-6 py-6 flex flex-col gap-5 font-body text-ink/90">
          {LINKS.map((l) => (
