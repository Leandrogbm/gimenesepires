...
Draft text the firm still needs to review: `Sobre.jsx` (partner bios), `Servicos.jsx` (the service cards), `FAQ.jsx` (the answers), and the `TODO` fields in `src/data/contato.js` (email, LinkedIn, OAB numbers). No real photos are used — colored initials (NG / NP) stand in for portraits. `src/components/Logo.jsx` is an SVG recreation of the logo; swap in the original vector if available.
